...
/**
 * Multiplies a blockdiagonal matrix with another, regular matrix in a more efficient manner.
 *
 * @param Blockdiagonal Matrix a
 * @param Matrix b
 * @param int blocksize
 * @return Matrix
 */
	...
	for(i = 0; i < a->rows; i++) {
		for(j = 0; j < b->cols; j++) {
			...
			for(k = 0; k < blocksize; k++) {
				sum = sum + m_get(a,i,k) * m_get(b,k+from,j);
			}
			m_set(c, i, j, sum);
			sum = 0;
		}
...
/**
 * Fills a blockdiagonal matrix with 1 on the main diagonal, 0 elswhere
 *
 * @param Matrix m
 * @return void
 */

void m_identityblockdiag(Matrix* m, int blocksize) {
	int i, j;

	assert(m->rows % blocksize == 0);

	for(i = 0; i < m->rows; i++) {
		for(j = 0; j < m->rows; j++) {
			if(i == j)
				m_set(m, i, j % blocksize, 1);
			else
				m_set(m, i, j % blocksize, 0);
		}
	}
}
/**
